    ...
    async with db as session:
        ...
        try:
            ...
            if user and user.password == user_data.password:
                ...
                publish_broker("INFO", f"[AUTH] Usuário com nome de [{user_data.username}] logado com sucesso!")
                ...
                publish_broker("ERROR", f"[AUTH] Tentativa de login com usuário: [{user_data.username}] falhou!")
                raise HTTPException(status_code=401, detail="Incorrect username or password")
        except NoResultFound:
            publish_broker("ERROR", f"[AUTH] Tentativa de login com usuário inexistente: [{user_data.username}]!")
...
async def get_users(db: Session = Depends(get_async_db)):
    ...
    publish_broker("INFO", f"[USERS] Exibição de todos os usuários requisitada!")
    return users
...
async def get_user(id: int, db: Session = Depends(get_async_db)):
    async with db as session:
        ...
        try:
            result = await session.execute(stmt)
            user = result.scalars().one()
            publish_broker("INFO", f"[USERS] Exibição de usuário com o id [{id}] requisitada!")
            return user
        except NoResultFound:
            publish_broker("ERROR", f"[USERS] Tentativa de exibição de usuário com o id [{id}] falhou!")
            raise HTTPException(status_code=404, detail="User not found")
...
async def create_user(user_data: UserInfo, db: Session = Depends(get_async_db)):
    ...
    publish_broker("INFO", f"[AUTH] Criação de usuário com o nome de [{user_data.username}] realizada!")
...
async def update_user(id: int, user_data: UserInfo, db: Session = Depends(get_async_db)):
    async with db as session:
        ...
        if not db_user:
            publish_broker("ERROR", f"[USERS] Tentativa de atualização de usuário com o id [{id}] falhou!")
            raise HTTPException(status_code=404, detail="User not found")
        db_user.name = user_data.username
        db_user.password = user_data.password
        await session.commit()

        publish_broker("INFO", f"[USERS] Usuário com o id [{id}] foi atualizado!")
...
async def delete_user(id: int, db: Session = Depends(get_async_db)):
    async with db as session:
        db_user = await session.get(User, id)
        if not db_user:
            publish_broker("ERROR", f"[USERS] Tentativa de delete de usuário com o id [{id}] falhou!")
            raise HTTPException(status_code=404, detail="User not found")
        await session.delete(db_user)
        await session.commit()
        publish_broker("INFO", f"[USERS] Usuário com o id [{id}] foi deletado!")
