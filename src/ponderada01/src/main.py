...
app.config["JWT_SECRET_KEY"] = "pokemon" 
...
def content():
    ...
    print(all_posts)
    return render_template("content.html", all_posts=all_posts)
...
def login():
    ...
    # recupera o token
    all_posts = http_request.get("http://localhost:5000/posts")
    response = make_response(render_template("content.html",  all_posts=json.loads(all_posts.content.decode("utf-8"))))
